...
starting_area_size = 5.5         	-- starting area in regions, safe from random nonsense

absolute_resource_chance = 0.40 -- chance to spawn an resource in a region
starting_richness_mult = 2.5		-- multiply starting area richness for resources
...
absolute_enemy_chance = 1	      -- chance to spawn enemies per sector (can be more then one base if spawned)
...
min_amount=8500 					-- default value for minimum amount of resource in single pile
...
resourceCollisionDetectionRatio = 0.8 -- threshold to exit placement early
resourceCollisionDetectionRatioFallback = 0.75 	-- at least this much of ore field needs to be placable to spawn it
resourceCollisionFieldSkip = false		-- determines if ore field should be skipped completely if placement based on ratio failed
