...
function InitSpawnGlobalsAndForces()
    ...
    gameForce.research_queue_enabled = true
...
function QueuePlayerForDelayedSpawn(playerName, spawn, moatEnabled)
    ...
    if ((spawn.x ~= 0) and (spawn.y ~= 0)) then
        ...
        local delay_spawn_seconds = 2*(math.ceil(ENFORCE_LAND_AREA_TILE_DIST/CHUNK_SIZE))

        --game.players[playerName].print("Generating your spawn now, " .. delay_spawn_seconds .. " seconds...")
...
function CreatePlayerCustomForce(player)
    ...
    elseif (TableLength(game.forces) < MAX_FORCES) then
		...
		newForce.worker_robots_storage_bonus=scenario.config.bots.worker_robots_storage_bonus;
		newForce.worker_robots_speed_modifier=scenario.config.bots.worker_robots_speed_modifier;
        if ENABLE_SHARED_TEAM_VISION then
