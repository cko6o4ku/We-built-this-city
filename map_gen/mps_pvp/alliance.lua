...
local function anarchy_gui(player)
	...
	local frame = player.gui.center.add({type = "frame", name = "anarchy_group_frame", direction = "vertical"})
	...
	local t = frame2.add({type = "table", name = "group_table", column_count = 4})
	...
	textfield.style.minimal_width = 40 + members_width * member_columns
	local b = t.add({type = "button", name = "main", caption = "Create - Spawn"})
	local b = t.add({type = "button", name = "far", caption = "Create - Random"}) -- <- enable this for random spawn
...
local function refresh_gui()
	for _, p in pairs(game.connected_players) do
		if p.gui.center["anarchy_group_frame"] then
			...
			local frame = p.gui.center["anarchy_group_frame"]
...
local function refresh_alliances()
	...
	for _, group in pairs(global.alliance_groups) do
		...
		if i == 0 then
			game.print('Group "' .. group.name .. '" has been abandoned!!', {r=191, g=78, b=78})
...
local function new_group(frame, player)								
	...
	if new_group_name ~= "" and new_group_name ~= "Name" and new_group_description ~= "Description" then
		...
		game.forces[new_group_name].technologies["atomic-bomb"].enabled = false
		--player.teleport({0, 0})
		local permission_group = game.permissions.get_group("Default")	
		permission_group.add_player(player.name)
		refresh_alliances()	
...
local function new_far(frame, player)		
    local surface =  game.surfaces[g_surface]
    local offset = {x=-0.5,y=0}						
	local new_group_name = frame.frame2.group_table.new_group_name.text
	local new_group_description = frame.frame2.group_table.new_group_description.text
	if new_group_name ~= "" and new_group_name ~= "Name" and new_group_description ~= "Description" then
		...
		init_player_weapon_damage(game.forces[new_group_name])
		game.forces[new_group_name].technologies["atomic-bomb"].enabled = false
        --game.forces[new_group_name].set_spawn_position(offset,surface)
        --ChartArea(game.forces[new_group_name], player.position, math.ceil(ENFORCE_LAND_AREA_TILE_DIST/CHUNK_SIZE), player.surface)
        ...
        if FRONTIER_ROCKET_SILO_MODE and ENABLE_SILO_VISION then
            ChartRocketSiloAreas(game.surfaces[g_surface], game.forces[new_group_name])
        end

        if FRONTIER_ROCKET_SILO_MODE then
        game.forces[new_group_name].set_friend("silo", true)
        game.forces["silo"].set_friend(new_group_name, true)
        game.forces[new_group_name].set_cease_fire("silo", true)
        game.forces["silo"].set_cease_fire(new_group_name, true)
    	end
        ...
        QueuePlayerForDelayedSpawn(tostring(player.name), newSpawn, moatChoice)
		refresh_alliances()
		player.gui.center["anarchy_group_frame"].destroy()
		return
	end			
...
local function ChangeForceSpawn(player, pos)
    global.playerSpawns[tostring(player.name)] = pos
    global.playerCooldowns[tostring(player.name)] = {setRespawn=game.tick}
end
...
local function on_gui_click(event)
	...
	if event.element.caption == "Accept" then
		...
		global.alliance_groups[group].members[tostring(requesting_player.name)] = requesting_player.index		
		game.print(tostring(requesting_player.name) .. ' has been accepted into group "' .. group .. '"', message_color)

    	ChangeForceSpawn(requesting_player, player.position)
		bring_player(requesting_player, player)	
		permission_group = game.permissions.get_group("Default")    
    	permission_group.add_player(requesting_player)		
		refresh_alliances()			
		destroy_request_guis(requesting_player)
		game.players[tostring(requesting_player.name)].gui.center["anarchy_group_frame"].destroy()			
		return
	end
	...
	if name == "far" then new_far(frame, player) return end
	...
	if p then
		if p.name == "groups_table" then
			if event.element.type == "button" and event.element.caption == "Join" then
						if (#game.forces[event.element.parent.name].connected_players == MAX_ONLINE_PLAYERS_AT_SHARED_SPAWN) then
							player.print("This group has reached it's maximum players.", {r=191, g=78, b=78})		
			...
			if event.element.type == "button" and event.element.caption == "Leave" then
				global.spam_protection[tostring(player.name)] = game.tick + 900
...
local function on_player_joined_game(event)
	...
	if player.online_time == 0 then
		player.force = game.forces.spectator
		player.print("Please join / create a group to play!", message_color)
		permission_group.add_player(tostring(player.name))
	end
