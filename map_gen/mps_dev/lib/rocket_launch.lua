...
function CreateRocketGuiTab(player, frame)
    -- local frame = frame.add{type="frame", name="rocket-panel", caption="Satellites Launched:", direction = "vertical"}
    frame.clear()
    AddLabel(frame, nil, "Satellites Launched:", my_label_header_style)

    if (global.satellite_sent == nil) then
        AddLabel(frame, nil, "No launches yet.", my_label_style)
    else
...
panel_tabs["Rockets"] = CreateRocketGuiTab