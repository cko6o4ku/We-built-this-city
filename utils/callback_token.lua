...
function Public._thread:valid(skip_location_check)
    ...
    if Server.is_type(self.uuid,'string') and
    _skip_location_check or Server.is_type(self.opened,'number') and
    _skip_location_check or Server.is_type(this.all[self.uuid],'table') and
    Server.is_type(self.timeout) or Server.is_type(self.timeout,'number') and
    Server.is_type(self.name) or Server.is_type(self.name,'string') and
    Server.is_type(self._close) or Server.is_type(self._close,'function') and
    Server.is_type(self._timeout) or Server.is_type(self._timeout,'function') and
    Server.is_type(self._tick) or Server.is_type(self._tick,'function') and
    Server.is_type(self._resolve) or Server.is_type(self._resolve,'function') and
    Server.is_type(self._success) or Server.is_type(self._success,'function') and
    Server.is_type(self._error) or Server.is_type(self._error,'function') then
        return true
    end
    return false
end
...
function Public._thread:close()
    ...
    local uuid = self.uuid
    local _return = false
    if Server.is_type(self._close,'function') then pcall(self._close,self) _return = true end
    ...
    if Server.is_type(self.name,'string') then threads.paused[self.name]=self.uuid self.opened=nil
...
function Public._thread:resolve(...)
    local _return = false
    if Server.is_type(self._resolve,'function') then 
        local success, err = pcall(self._resolve,self,...)
        if success then
            if Server.is_type(self._success,'function') then
...
function Public._thread:check_timeout()
    ...
    if Server.is_type(self.timeout,'number') and game.tick >= (self.opened+self.timeout) then
        if Server.is_type(self._timeout,'function') then
...
function Public._thread:error(err)
    local _return = false
    if Server.is_type(self._error,'function') then
...
function Public._thread:on_event(event,callback)
    ...
    if value and Server.is_type(callback,'function') then
        self['_'..value] = callback
    elseif Server.is_type(event,'number') and Server.is_type(callback,'function') then
...
function Public._thread_handler(event)
    ...
    Table.each(threads,function(uuid)
        local next_thread = Public.get_thread(uuid)
        if next_thread and next_thread:valid() then
            if Server.is_type(next_thread._events[event_id],'function') then
...
function Public.interface(callback,use_thread,...)
    if use_thread then
        ...
        use_thread:on_event('resolve',function(thread)
            if Server.is_type(thread.data[1],'function') then
                local success, err = pcall(unpack(thread.data))
                if not success then error(err) end
                return err
            else
                local callback = thread.data[1] == nil
        ...
        if Server.is_type(callback,'function') then
...
commands.add_command('interface', 'Runs the given input from the script', function(args)
    ...
    if not success and Server.is_type(err,'string') then local _end = string.find(err,'stack traceback') if _end then err = string.sub(err,0,_end-2) end end
