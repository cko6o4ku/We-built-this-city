...
local entities_that_earn_raw_fishs = {
...
local function on_entity_died(event)	
	...
	if not raw_fish_yield[event.entity.name] then return end
	...
	if event.cause then
		if event.cause.valid then
			...
			for _, player in pairs(players_to_reward) do
				player.insert({name = "raw-fish", count = raw_fish_yield[event.entity.name]})
			end
		end
		if entities_that_earn_raw_fishs[event.cause.name] then
			event.entity.surface.spill_item_stack(event.cause.position,{name = "raw-fish", count = raw_fish_yield[event.entity.name]}, true)
			reward_has_been_given = true
		end
	end	
																	
	if reward_has_been_given == false then
		event.entity.surface.spill_item_stack(event.entity.position,{name = "raw-fish", count = raw_fish_yield[event.entity.name]}, true) 
	end
end
